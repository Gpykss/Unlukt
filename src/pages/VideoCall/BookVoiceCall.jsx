// src/pages/VoiceCall/BookVoiceCall.jsx
import { useState, useEffect } from 'react';
import { useParams, useNavigate } from 'react-router-dom';
import { motion } from 'framer-motion';
import { ArrowLeft, Phone, Clock, DollarSign, Calendar, CheckCircle, Loader2, User } from 'lucide-react';
...
export default function BookVoiceCall() {
 ...
 const durations = [
  ...
  const getPrice = () => {
  const basePrice = Math.max(12, creator?.voiceCallPrice || 12);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        ...
        >
          ...
          <p className="text-gray-600 mb-6">Your voice call has been booked. You'll receive a notification when the creator confirms.</p>
          <button
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-5">
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-5 flex items-center space-x-4">
            ...
            <div>
              ...
              <div className="flex items-center space-x-1 mt-1">
                ...
                <span className="text-sm text-purple-600 font-semibold">${creator.voiceCallPrice} / 30 min</span>
              </div>
            </div>
          </div>
        )}
        ...
        <button
          onClick={handleBook}
          className="w-full py-4 bg-purple-500 hover:bg-purple-600 text-white rounded-xl font-bold text-lg transition shadow-lg"
        >
          Book Now — ${getPrice()}
        </button>
      </div>
      ...
      <PaymentModal
        ...
        contentId={`voice_${creatorId}_${Date.now()}`}
        creatorId={creator?.id}
