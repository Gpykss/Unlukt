...
export default function SubscribeModal({ isOpen, onClose, creator, onSuccess }) {
  ...
  useEffect(() => {
    ...
    if (creator?.uid) {
      // Load the most relevant active discount
      getCreatorDiscount(creator.uid).then(data => {
        if (!data) return;
        // Priority: limited_time > first_month > bundle
        const active =
            ...
            <>
              ...
              <div className="flex items-center justify-between px-5 pt-5 pb-3">
                <div className="flex items-center space-x-3">
                  <div className="w-11 h-11 rounded-full overflow-hidden bg-gradient-to-br from-rose-100 to-pink-200 flex items-center justify-center text-xl">
                    {creator.avatar
                  ...
                  >
                    {subscribing
                      ...
                      : <>
                          ...
                          <span>Subscribe • ${selectedPrice.toFixed(2)}</span>
                        </>
                    }
                  </button>
                )}
